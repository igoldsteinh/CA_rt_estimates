...
ca_dat <- dat %>%
          ...
          summarise(cases = sum(cases),
                    est_cases = sum(est_cases),
                    est_omicron_cases = sum(est_omicron_cases),
                    est_other_cases = sum(est_other_cases),
                    tests = sum(tests),
                    est_tests = sum(est_tests),
                    est_omicron_tests = sum(est_omicron_tests),
                    est_other_tests = sum(est_other_tests),
                    hospitalizations = sum(hospitalizations),
                    deaths = sum(deaths),
                    est_deaths = sum(est_deaths),
                    icu = sum(icu),
                    cum_deaths = sum(cum_deaths),
                    cum_est_deaths = sum(cum_est_deaths),
                    prev_cum_deaths = sum(prev_cum_deaths),
                    prev_cum_est_deaths = sum(prev_cum_est_deaths)) %>%
            mutate(county = "California")
  # to dos, merge with county data
  # add extra row to county ids
